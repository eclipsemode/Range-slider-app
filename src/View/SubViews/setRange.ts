...
function calcMouseOffset(mouseOffset: number, sliderWidth: number): number {
    if (mouseOffset < 0) {
        return 0;
    } else if (mouseOffset > sliderWidth) {
        return sliderWidth;
    } else {
        return mouseOffset;
    }
    ...
    if (isRangeTrue) {
        ...
        $thumbMin.on('mousedown', (e) => {
            ...
            const moveAt = (e: JQuery.MouseMoveEvent | JQuery.MouseDownEvent) => {
                ...
                const thumbOffsetValue: number = calcMouseOffset(mouseOffset, sliderWidth);

                this.opts.from =
                    Math.ceil(progressCalc(e, sliderWidth, $barLine)) / 100 * (this.opts.max - this.opts.min) - this.opts.max;
