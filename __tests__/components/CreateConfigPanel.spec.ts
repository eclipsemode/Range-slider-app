...
import CreateConfigPanel from "../../src/View/SubViewsOld/configPanel/CreateConfigPanel";
...
const newConfigPanel = new CreateConfigPanel(
  newSelector,
  newControlConfig,
  newToggleConfig
);
...
describe("getConfig method", () => {
  ...
  //   const f = (CreateConfigPanel.prototype.getConfig = jest.fn());
  //
  //   f();
  //
...
describe("getControls method", () => {
  test("Should have an id", () => {
    newControlConfig.forEach((name) => {
      expect(
        $(".slider-app__config-section--control").find(
          $(`#slider__control-${name}`)
        )
      ).toBeTruthy();
    });
  });
